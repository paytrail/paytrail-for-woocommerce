...
final class Gateway extends \WC_Payment_Gateway {
    ...
    /** Pay and add card tokeniztion method
    */
	...
	/**
	 * Process card token for pay-and-add-card method
	 *
	 * @return bool
	 * @throws HmacException
	 * @throws ValidationException
	 */
	public function process_pay_add_card_token($token,$user_id) {
		$getTokenRequest = new GetTokenRequest();
      
		$getTokenRequest->setCheckoutTokenizationId($token);
      
		$this->log('Paytrail: process_card_token for pay and add card method', 'debug');

		$response = $this->client->createGetTokenRequest($getTokenRequest);

		return (bool) $this->save_pay_add_card_token($response,$user_id);
	}  
	 ...
	 */
	private function save_pay_add_card_token(GetTokenResponse $card_token,$user_id) {
	...
	public function add_card_form( $context = Plugin::ADD_CARD_CONTEXT_CHECKOUT) {
		...
		// @var \GuzzleHttp\Psr7\Response $response createPaymentAndAddCard
		$response = $this->client->createAddCardFormRequest($add_card_form_request);
         //$response = $this->client->createPaymentAndAddCard($payment);
	...
	public function handle_payment_response( $status) {
		...
		if (empty($orders)) {
            add_post_meta(15230, '_paycard_response', $get);
	...
	public function process_payment( $order_id) {
        ...
        if($is_pay_and_card){
              $this->add_card_form_and_pay($payment, $order);
          print_r("tes");exit;
        }
		...
		try {
			...
			$order->add_order_note(
				sprintf(
					...
					__('Refunding ok process started.%s', 'paytrail-for-woocommerce'),
			...
			add_action(
				...
				function ( $order_id, $refund_id) use ( $order, $refund, $reason, $transaction_id, $amount, $price, $refund_unique_id) {
                    ...
                    //add_post_meta($refund_object->get_id(),'order_item_response',$refund_object);  
